...
set.seed(16)
rownames(rmo) <- c("D", "M", "E", "X") #participants
colnames(rmo) <- c("no,G1", "yes, G1", "no, G2", "yes,G2") 
rmo
...
#Input Data
mem_score <- c(12, 12, 10, 13,
               12, 14, 14, 11,
               10, 9, 8, 9,
               7, 10, 8, 8)

participant <- rep(c("D", "X", "M", "E",
                     "X", "E", "D", "M",
                     "E", "D", "X", "M",
                     "M", "X", "E", "D"))

mgame <- c(rep(c(1,2), each = 8))
              
days <- c(rep(c(1,2), each = 4, times = 2)) #FIX
...
###############################################################
#Additional Design: 2^3 Factorial w/ replicates in 4 blocks
###############################################################
